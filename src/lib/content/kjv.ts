...
import type { ParseResult, ScriptureBookMeta } from "./scripture";
import {
  getBookMeta,
...
export type KjvBookMeta = ScriptureBookMeta;
...
export function getKjvBookMeta(abbrevOrName: string): KjvBookMeta | undefined {
  return getBookMeta(KJV_BOOKS, abbrevOrName);
}
...
export function sliceKjvPassage(
  ...
  return slicePassage(book, chapter, verseStart, verseEnd, endChapter);
}

export function parseDolLessonRef(ref: string): ParseResult | null {
  return parseDolRef(KJV_BOOKS, ref);
}

export async function getKjvPassageFromDolRef(
  ref: string,
): Promise<KjvPassage | null> {
  return getPassageFromDolRef(KJV_BOOKS, loadKjvBook, ref);
}
...
export async function getKjvPassagesFromDolRef(
  ...
  return getPassagesFromDolRef(KJV_BOOKS, loadKjvBook, ref);
}
